...
async function recomputePartyStats(partyId) {
  ...
  const reputationScore =
    reviews.length > 0 ? round((reviews.reduce((s, r) => s + r.rating, 0) / reviews.length) * 20) : 0;
  ...
  const finishedDeals = deals.filter((d) => ['COMPLETED', 'CANCELLED', 'EXPIRED'].includes(d.status));
  ...
  return { reputationScore, responseScore, completionScore, activityScore, cancelCount, expiredCount };
}
